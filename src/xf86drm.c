 ...
 */
static int drmOpenDevice(int type)
{
    stat_t          st;
...
wait_for_udev:
    ...
    fd = open(DRM_DIR_NAME, O_RDWR | O_CLOEXEC, 0);
    if (fd <= 0) {
        printf("%s:%d: open %s is failed!\n", __func__, __LINE__, DRM_DIR_NAME);
        return -1;
    }
...
static int drmOpenByBusid(const char *busid, int type)
{
    ...
    _PrintFormat("drmOpenByBusid: Searching for BusID %s\n", busid);
    for (i = base; i < base + DRM_MAX_MINOR; i++) {
        fd = drmOpenMinor(i, 1, type);
        _PrintFormat("drmOpenByBusid: drmOpenMinor returns %d\n", fd);
        if (fd >= 0) {
            ...
            if (drmSetInterfaceVersion(fd, &sv)) {
                ...
                sv.drm_dd_major = -1;       /* Don't care */
                sv.drm_dd_minor = -1;       /* Don't care */
                _PrintFormat("drmOpenByBusid: Interface 1.4 failed, trying 1.1\n");
                drmSetInterfaceVersion(fd, &sv);
            }
            buf = drmGetBusid(fd);
            _PrintFormat("drmOpenByBusid: drmGetBusid reports %s\n", buf);
        ...
        if ((fd = drmOpenMinor(i, 1, type)) >= 0) {
            if ((version = drmGetVersion(fd))) {
                if (!strcmp(version->name, name)) {
                    drmFreeVersion(version);
                    id = drmGetBusid(fd);
                    _PrintFormat("drmGetBusid returned '%s'\n", id ? id : "NULL");
                    if (!id || !*id) {
                        if (id)
                            drmFreeBusid(id);
                        return fd;
...
drm_public int drmOpenWithType(const char *name, const char *busid, int type)
{
    ...
    if (name != NULL && drm_server_info &&
        ...
        if (!drm_server_info->load_module(name)) {
            _PrintFormat("[drm] failed to load kernel module \"%s\"\n", name);
            return -1;
        }
    }
...
drm_public drmVersionPtr drmGetVersion(int fd)
{
    ...
    printf("SylixOS custom version, not Support major and minor!\r\n");
    return 0;
#if 0
    ...
    if (drmIoctl(fd, DRM_IOCTL_VERSION, version)) {
        _PrintFormat("DRM_IOCTL_VERSION: %s\n", strerror(errno));
        drmFreeKernelVersion(version);
        return NULL;
    }
                         ...
                         drmMapFlags flags, drm_handle_t *handle)
{
    ...
    memclear(map);
    map.offset  = (unsigned long)offset;
    map.size    = (unsigned long)size;
...
//Mjj mmap! here may be reset fd size, if not will lead to failed! to be test

    *address = drm_mmap(0, size, PROT_READ|PROT_WRITE, MAP_SHARED, fd, handle);
    if (*address == MAP_FAILED) {
        printf("%s:%d mmap size failed\r\n", __func__, __LINE__);
        return -errno;
    }
...
static char *drmGetMinorNameForFD(int fd, int type)
{
    ...
    return (char *)DRM_PRIMARY_MINOR_NAME;
}
...
/*
    need write pci info   
        /sys/dev/char/226:0/device/config 
        /sys/dev/char/226:0/device/uevent
        /sys/dev/char/226:0/device/revision,
        /sys/dev/char/226:0/device/vendor
        /sys/dev/char/226:0/device/device
        /sys/dev/char/226:0/device/subsystem_vendor
        /sys/dev/char/226:0/device/subsystem_device
*/
               ...
               bool fetch_deviceinfo, uint32_t flags)
{
    ...
    int node_type = 0, subsystem_type = 0;
    unsigned int maj = 226, min = 0;
#if 0
...
/*
    /sys/dev/char/226:0/uevnet -> /drm/dri/card0
    path = "/drm/dri/card0"
*/
    return (char *)DRM_DIR_NAME;
}
